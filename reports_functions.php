...
function printBockerForElev($elevId){
	...
	print "<h2>Böcker</h2>";
	print "<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
	print "<tr><th>Titel</th><th>Författare</th><th>För kurs</th><th>Utdelas</th><th>Tillbaka</th></tr>";
	foreach($bokArr as $bok){
		print "<td>" . getFullBokTitelFromId($bok["isbn"]) . "</td>";
		print "<td>" . $bok["forf_fornamn"] . " " . $bok["forf_efternamn"] . "</td>";
		print "<td>" . $bok["kursid"] . "</td>";
		print "<td>" . getTillfalleDescLongForId($bok["ut_tid_id"]) . "</td>";
		print "<td>" . getTillfalleDescLongForId($bok["in_tid_id"]) . "</td></tr>";
	}
	print "</table>";
